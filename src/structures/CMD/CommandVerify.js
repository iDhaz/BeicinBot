...
module.exports = class CommandVerify {
    ...
    VERIFY_CLIENT({ channel, command, author, t }) {
        try {
            ...
            if (PERMS_VERIFY.some(perm => perm.VerifyBoolean === false)) {

                let PERMS_LENGTH = PERMS_VERIFY.filter(perm => perm.VerifyBoolean === false)
                let NEED_PERMS = PERMS_VERIFY.filter(perm => perm.VerifyBoolean === false).map(perm => `**"${perm.PermName}"**`).join(", ")
                let ERR_USAGE = null
